...
import { capitalazeFirstLetter } from '../../app/utils/capitalazeFirstLetter'
// import { format } from 'date-fns'

interface DatePickerPropvalue {
  value: Date
  onChange?(date: Date): void
}

export function DatePicker({ value, onChange }: DatePickerPropvalue) {
  return (
    <DayPicker
      locale={ptBR}
      selected={value}
        ...
        formatCaption: (month, options) => {
          return (
            <span className='text-gray-900 tracking-[-0.408px] font-medium'>
              {capitalazeFirstLetter(format(month, 'LLLL yyyy', options))}
            </span>
          )
        }
      }}
